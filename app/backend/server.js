...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    db.query(sql, [username, email, hashedPassword], (err, result) => {
      if (err) return res.status(500).json({ error: 'Eroare la înregistrare' });
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [email], async (err, results) => {
    ...
    if (results.length === 0) return res.status(401).json({ error: 'Email invalid' });

    const user = results[0];
    const match = await bcrypt.compare(password, user.password);
    if (!match) return res.status(401).json({ error: 'Parolă incorectă' });

    res.status(200).json({ message: 'Autentificare reușită', user: { id: user.id, username: user.username } });
  });
});
